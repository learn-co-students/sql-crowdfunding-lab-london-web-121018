...
"SELECT projects.title, SUM(amount) FROM projects, pledges GROUP BY pledges.project_id, projects.title HAVING pledges.project_id = projects.id ORDER BY projects.title";
end

def selects_the_user_name_age_and_pledge_amount_for_all_pledges_alphabetized_by_name
"SELECT users.name, users.age, SUM(amount) FROM users, pledges ON users.id = pledges.user_id GROUP BY pledges.user_id ORDER BY users.name;"
end
...
def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
  ...
  HAVING projects.id = pledges.project_id ORDER BY SUM(amount) - Projects.funding_goal < -1 ASC LIMIT 2;"
end
